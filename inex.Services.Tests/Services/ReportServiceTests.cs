...
public class ReportServiceTests
{
    ...
    [Fact]
    public async Task GetMonthlyFinancialReport_IncludesAllActiveAccountsWhenNoSelectionIsSupplied()
    {
        var incomeCategory = Category(id: 10, name: "Salary");
        var expenseCategory = Category(id: 11, name: "Groceries");
        var service = CreateService(
        ...
        Assert.Equal(1_300m, report.TotalIncome);
        Assert.Equal(570m, report.TotalExpenses);
        Assert.Equal(50m, report.OpeningBalance);
        Assert.Equal(780m, report.ClosingBalance);
    }
    ...
    [Fact]
    public async Task GetMonthlyFinancialReport_RejectsAnEmptyExplicitAccountSelection()
    {
        var service = CreateService(
            categories: [Category(id: 10, name: "Salary")],
            accounts: [Account(id: 20, currency: "USD")],
        ...
        await Assert.ThrowsAsync<ValidationFailedException>(() => service.GetMonthlyFinancialReport(UserId, 2026, 5, accountIds: []));
    }
